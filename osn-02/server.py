...
from packages import tcp_socket, server_thread
...
def main():
  ...
  welcomingSocket = tcp_socket.tcp_socket()
  welcomingSocket.initServerSocket(address)

  # create keyboard intrupt, Ctrl-C, using SIGINT 
  signal.signal(signal.SIGINT, handler)

  # create dict object for server data and a lock to be used across threads
  serverdata = {}
  ...
  while True:
    ...
    clientSocket, _ = welcomingSocket.acceptClientConnection()

    # thread to handle client socket
    server_thread.server_thread(clientSocket, serverdata, datalock).start()
