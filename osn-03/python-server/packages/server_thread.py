...
def register(sock, data, lock):
  ...
  # await client registration info
...
def search(keyword, sock, data, lock):
  ...
  if matching:
    sock.sendMessage(' '.join(matching) + '\n')
  else:
...
class server_thread(threading.Thread):
  ...
  def run(self):
    ...
    while not exit:
      ...
      command = self.sock.receiveMessage().replace('\x00', '').split()
      if(command):
        
        # if command is 'SEARCH' search server data for keyword and send match(es)
        if command[0].upper() == 'SEARCH' and len(command) == 2:
         search(command[1], self.sock, self.data, self.lock)
